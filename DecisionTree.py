...
def find_best_split(rows, header):
    ...
    #print("Inside find_best_split Function")
    #print("Rows: ", rows)
    ...
    #print("best Gain: ",best_gain, " best question: ", best_question)
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.id = id
        self.rows = rows
        self.predictedClassLabel = label_counts(rows)
...
def build_tree(rows, header, depth=0, id=0):
    ...
    #print("Inside build_tree Function")
    #print("rows: ", rows)
...
def prune_tree(node, prunedList):
    ...
    giant stack traces.
    """
    #print ([node.id for node in prunedList])
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        #Updated by Ekansh
        print (spacing + "Predict", node.predictions, spacing + "predicted_class_label", node.predictedClassLabel)
        return

    # Print the question at this node
    #Updated by Ekansh
    print (spacing + str(node.question), spacing + "depth", node.depth, spacing + "node_id", node.id)

    # Call this function recursively on the true branch
    #Updated by Ekansh
    print (spacing + '--> True:', spacing + "  ", spacing + "depth", node.depth, spacing + "node_id", node.id)
    print_tree(node.true_branch)

    # Call this function recursively on the false branch
    #Updated by Ekansh
    print (spacing + '--> False:', spacing + "  ", spacing + "depth", node.depth, spacing + "node_id", node.id)
